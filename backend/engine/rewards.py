...
# ── Forensic Reward Constants (VaultAgent — APK malware analysis) ──
CORRECT_FLAG_REWARD = +0.30     # each correctly-detected malware indicator (capped)
EVIDENCE_BONUS = +0.20          # conclusion is backed by concrete code/permission evidence
CORRECT_FAMILY_REWARD = +0.25   # malware family classified correctly
SCHEMA_VALID_REWARD = +0.10     # output is valid machine-readable JSON
HALLUCINATED_PERMISSION_PENALTY = -0.60  # cited a permission not in the manifest
WRONG_CLASSIFICATION_PENALTY = -0.50     # benign↔malicious or wrong family
MISSED_INDICATOR_PENALTY = -0.40         # failed to flag an obvious indicator
FLAG_REWARD_CAP = +0.90         # ceiling on summed correct-flag reward
...
class RewardCalculator:
    ...
    # ── Forensic Reward Functions (VaultAgent) ──

    def correct_flags(self, n_correct: int, agent: str = "CODER") -> float:
        """Reward each correctly-detected malware indicator, capped."""
        value = min(CORRECT_FLAG_REWARD * max(0, n_correct), FLAG_REWARD_CAP)
        return self._log(f"Correct flags ({n_correct})", value, agent=agent, tag="FLAGS")

    def evidence_backed(self, agent: str = "CODER") -> float:
        """Reward an evidence-backed conclusion."""
        return self._log("Evidence-backed", EVIDENCE_BONUS, agent=agent, tag="EVIDENCE")

    def correct_family(self, agent: str = "COMMANDER") -> float:
        """Reward correct malware-family classification."""
        return self._log("Correct family", CORRECT_FAMILY_REWARD, agent=agent, tag="FAMILY")

    def schema_valid(self, agent: str = "") -> float:
        """Reward valid machine-readable JSON output."""
        return self._log("Schema valid", SCHEMA_VALID_REWARD, agent=agent, tag="SCHEMA")

    def hallucinated_permission(self, count: int = 1, agent: str = "DETECTIVE") -> float:
        """Penalize citing permissions not present in the manifest."""
        value = HALLUCINATED_PERMISSION_PENALTY * max(1, count)
        return self._log(f"Hallucinated permission ({count})", value, agent=agent, tag="HALLUC")

    def wrong_classification(self, agent: str = "COMMANDER") -> float:
        """Penalize wrong benign/malicious verdict or wrong family."""
        return self._log("Wrong classification", WRONG_CLASSIFICATION_PENALTY, agent=agent, tag="MISCLASS")

    def missed_indicator(self, count: int = 1, agent: str = "DETECTIVE") -> float:
        """Penalize missing an obvious indicator."""
        value = MISSED_INDICATOR_PENALTY * max(1, count)
        return self._log(f"Missed indicator ({count})", value, agent=agent, tag="MISSED")

    def score_forensic_report(
        self,
        *,
        predicted_flags: set,
        expected_flags: set,
        manifest_permissions: set,
        cited_permissions: set,
        ...
        """
        End-to-end scoring of one forensic report against ground truth.
        Mirrors the GRPO reward shaping so live runs and training agree.
        """
        start = self.total_reward
        correct = predicted_flags & expected_flags
        missed = expected_flags - predicted_flags
        hallucinated_perms = cited_permissions - manifest_permissions

        if correct:
            self.correct_flags(len(correct), agent=agent)
        if missed:
            self.missed_indicator(len(missed), agent=agent)
        if hallucinated_perms:
            self.hallucinated_permission(len(hallucinated_perms), agent=agent)
        if has_evidence:
            self.evidence_backed(agent=agent)
        if schema_ok:
            self.schema_valid(agent=agent)
        if expected_family:
            if predicted_family and predicted_family.lower() == expected_family.lower():
                self.correct_family(agent=agent)
            elif predicted_family:
                self.wrong_classification(agent=agent)

        return round(self.total_reward - start, 3)
