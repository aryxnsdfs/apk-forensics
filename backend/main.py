...
from engine.apk_extractor import ApkExtractor
from engine import apk_features
from schemas.forensic import validate_verdict
...
async def demo_simulation_loop(scenario_id: str):
    ...
    if scenario_id == "malware_apk":
        # VaultAgent scripted malware-analysis storyline (no LLM required).
        events = [
            (2.0, "INFO", "swarm-os.apk", "[EXTRACT] Androguard parsed com.flashlight.app — 1.2MB, 5 permissions, 1 exported receiver"),
            (1.5, "WARNING", "swarm-os.apk", "[HEURISTIC] Suspicious combo RECEIVE_SMS+INTERNET detected"),
            (1.5, "INFO", "swarm-os.orchestrator", "Agent 'DETECTIVE' (Static Analyst) broadcast: [STATIC | RECEIVE_SMS+INTERNET | REQ_CODE:SmsRx]"),
            (2.0, "INFO", "swarm-os.orchestrator", "Agent 'CODER' (Reverse Engineer) broadcast: [REVERSE | FLAG_SMS_THEFT | FLAG_C2 | obf=false]"),
            (1.0, "INFO", "swarm-os.apk", "[CODE] SmsRx.onReceive() forwards SMS body to http://192.0.2.10/gate"),
            (2.0, "INFO", "swarm-os.orchestrator", "Agent 'COMMANDER' (Chief Security Officer) broadcast: [VERDICT | THREAT_HIGH | SMS-Stealer | FLAG_SMS_THEFT FLAG_C2 | ETA_10s]"),
            (1.5, "INFO", "swarm-os.causal", "Auto-generating Root Cause Analysis report: vaultagent_rca_report.md"),
        ]
    elif scenario_id == "sql_deadlock":
        events = [
            # The Trigger
...
# ════════════════════════════════════════════════════════════════════
#  VaultAgent — Offline APK Malware Forensic Pipeline
#  APK upload → Androguard extract → Static Analyst → Reverse Engineer →
#  Chief Security Officer (verdict + RCA) → attack DAG → dashboard.
#  Fully offline: uses the local GGUF model when reachable, otherwise a
#  deterministic heuristic stub so the dashboard always populates.
# ════════════════════════════════════════════════════════════════════

APK_UPLOAD_DIR = Path(os.getenv("VAULTAGENT_UPLOAD_DIR", "/tmp/vaultagent_apks"))


def _apk_telemetry(metadata: dict, seed: dict) -> dict:
    """Map APK metrics onto the existing monitor payload (relabelled in UI)."""
    return {
        "ram": metadata.get("file_count", 0),            # extracted files
        "vram": round(seed.get("threat_score", 0) / 25.0, 2),  # threat gauge 0-4
        "network": len(metadata.get("urls", [])) + len(metadata.get("ips", [])),
        "cpu": len(metadata.get("permissions_dangerous", [])),
        "containerStatus": "critical" if seed.get("threat_score", 0) >= 40 else "running",
    }


async def _stage_llm_or_stub(agent_role: str, prompt: str, stub: str) -> str:
    """Use the local GGUF model if live and reachable, else return the stub."""
    if DEMO_MODE or llm_inference is None or llm_inference.mock_mode:
        return stub
    try:
        sys_prompt = orchestrator.active_agents.get(agent_role, {}).get("system_prompt", "")
        out = llm_inference.generate(prompt, agent_role, system_prompt=sys_prompt, max_tokens=512)
        text = (out or {}).get("response", "").strip()
        return text or stub
    except Exception:
        logger.exception("LLM stage failed for %s; using heuristic stub", agent_role)
        return stub


async def _run_apk_analysis(apk_path: str, display_name: str):
    """Sequential 3-stage forensic pipeline streamed over the WebSocket."""
    causal_engine.reset()
    reward_calculator.reset()
    orchestrator.reset()
    _reset_frontend_replay_buffer()

    task_id = "apk_static_analysis"
    await broadcast({"type": "tasks_queued", "payload": {"task_ids": [task_id]}})
    ...
    snippets = ""
    for cls in seed.get("code_requests", [])[:3]:
        snippets += f"\n// {cls}\n" + (extractor.get_method_smali(cls) or "[no decompiled body available]\n")
    rev_flags = [f for f in seed.get("flags", []) if f not in ("FLAG_EXPORTED_SURFACE",)]
    rev_stub = (
        f"Behaviors inferred from requested code: "
        f"{', '.join(b['detail'] for b in seed.get('evidence', [])) or 'no malicious behavior confirmed'}. "
    ...
    verdict = validate_verdict({
        "threat_score": seed.get("threat_score", 0),
        "threat_level": seed.get("threat_level", "THREAT_BENIGN"),
        "malware_family": _guess_family(seed.get("flags", [])),
    ...
    cso_stub = (
        f"Verdict: {verdict['threat_level']} (score {verdict['threat_score']}). "
    ...
    await broadcast({"type": "rca_document", "payload": rca_md})
    await broadcast({"type": "scenario_complete", "payload": {"task_id": task_id, "verdict": verdict}})
    logger.info("VaultAgent analysis complete for %s -> %s (%d)",
                display_name, verdict["threat_level"], verdict["threat_score"])
    return verdict


def _static_prompt(metadata: dict, seed: dict) -> str:
    return (
        "Analyze this APK manifest metadata as the Static Analyst. Identify dangerous "
        "permissions, exported components, and suspicious combinations. List code to review.\n"
        f"package={metadata.get('package')}\n"
        f"permissions={metadata.get('permissions')}\n"
        f"exported={metadata.get('exported_components')}\n"
        f"ips={metadata.get('ips')} urls={metadata.get('urls')}\n"
    )


def _reverse_prompt(metadata: dict, seed: dict, snippets: str) -> str:
    return (
        "As the Reverse Engineer, explain the malicious intent of these code snippets. "
        "Detect credential theft, OTP/SMS interception, exfiltration, obfuscation, and C2.\n"
        f"flags_so_far={seed.get('flags')}\nCODE:\n{snippets[:6000]}\n"
    )


def _verdict_prompt(metadata: dict, seed: dict) -> str:
    return (
        "As the Chief Security Officer, issue the final verdict JSON {threat_score, "
        "threat_level, malware_family, indicators, evidence, rca, mitigation} for this APK.\n"
        f"package={metadata.get('package')}\nseed={json.dumps(seed)}\n"
    )


_FAMILY_BY_FLAG = [
    ("FLAG_SMS_THEFT", "SMS-Stealer"),
    ("FLAG_OVERLAY_ATTACK", "Banking-Overlay"),
    ("FLAG_DEVICE_ADMIN_ABUSE", "Device-Admin-Abuser"),
    ("FLAG_DROPPER", "Dropper"),
    ("FLAG_SPYWARE", "Spyware"),
    ("FLAG_CREDENTIAL_STEALING", "Credential-Stealer"),
]


def _guess_family(flags: list) -> str:
    fset = set(flags)
    for flag, family in _FAMILY_BY_FLAG:
        if flag in fset:
            return family
    return "Unknown"
...
async def analyze_apk(file: UploadFile = File(...)):
    """Upload an APK and run the offline forensic pipeline. Streams to the dashboard."""
    global scenario_task
    await _stop_scenario_task()

    APK_UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
    safe_name = os.path.basename(file.filename or "upload.apk")
    dest = APK_UPLOAD_DIR / safe_name
    content = await file.read()
    with open(dest, "wb") as fh:
        fh.write(content)
    logger.info("POST /api/analyze -> saved %s (%d bytes)", dest, len(content))
